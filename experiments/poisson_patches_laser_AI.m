function code = poisson_patches_laser_AI
...
try
  ...
  % receive copy of data sent to laser & galvos (sync)
  vr.laser.dataIn = nidaqAIread('AIread');
...
binPos = dec2bin(vr.laser.currentState,8); 
for jj = 1:numel(RigParameters.locationChannels)
    vr.laser.posOut(jj) = str2double(binPos(jj));
end
...
vr.predictedYpos = vr.position(2)+vr.velocity(2)*vr.laser.rampDownTime;
