...
function [mainMazeID, mazeID, warmupIndex, prevPerformance] = getTrainingLevel(mazes, trainee, console, numMazes, doAutoAdvance)
  ...
  if trainee.overrideMazeID || isnan(lastDay)
    ...
    if isempty(criteria.warmupMaze) || trainee.overrideMazeID > 1
      mazeID        = mainMazeID;
    else
      warmupIndex   = 1;
  ...
  % Check if the animal has reached criterion for the last maze that was run
    ...
    for iDay = lastButToday:-1:1
      ...
      if isfield(criteria, 'criteriaNTrials') && (numel(block(end).trialType) >= criteria.criteriaNTrials)
        % If a maximum number of trials to use for evaluating the criteria is 
        % provided, select the best (highest performance) subset of trials to use 
        nKernel     = ceil(criteria.criteriaNTrials / 2);
        kernel      = ones(1,nKernel) / nKernel;
        choice      = block(end).choice;
        trialType   = block(end).trialType;
        isCorrect   = ( choice == trialType );
        iRight      = find(trialType == Choice.R);
        iLeft       = find(trialType == Choice.L);
        performR    = interp1(iRight, filter(kernel, 1, isCorrect(iRight)), 1:numel(trialType), 'linear');
        performL    = interp1(iLeft , filter(kernel, 1, isCorrect(iLeft )), 1:numel(trialType), 'linear');
        performR    = performR(criteria.criteriaNTrials:end);
        performL    = performL(criteria.criteriaNTrials:end);
        performance = mean([performR; performL], 1);
        bias        = abs(performR - performL);
        [~,iPerform]= find(performance >= criteria.performance & bias < criteria.maxBias, 1, 'first');
        if isempty(iPerform)
          [~,iPerform]  = max(performance);
        end
        performance = performance(iPerform);
        bias        = bias(iPerform);
      else
        % Otherwise use all trials
...
function block = getTrainingBlocks(run)
  ...
  for iRun = 1:numel(run)
    ...
    hasEasyBlocks     = isfield(run(iRun).block, 'easyBlockFlag');
    for iBlock = 1:numel(run(iRun).block)
      if      ~run(iRun).block(iBlock).isActive             ...
          ||  isempty(run(iRun).block(iBlock).trialType)    ...
          ||  (hasEasyBlocks && run(iRun).block(iBlock).easyBlockFlag)
        continue;
      end
      
      if isempty(block)
        block         = run(iRun).block(iBlock);
      elseif      block(end).mazeID       == run(iRun).block(iBlock).mazeID                     ...
              &&  block(end).mainMazeID   == run(iRun).block(iBlock).mainMazeID
            
        more          = run(iRun).block(iBlock);
        range         = numel(block(end).trialType) + (1:numel(more.trialType));
        type          = 1:numel(more.performance);
        block(end).numTrials          = block(end).numTrials + more.numTrials;
        block(end).performance        = ( block(end).performance .* block(end).numTrials(type)  ...
                                        + more.performance       .* more.numTrials(type)        ...
                                        )                                                       ...
                                     ./ ( block(end).numTrials(type) + more.numTrials(type) )   ...
                                      ;
        block(end).trialType(range)   = more.trialType;
        block(end).choice(range)      = more.choice;
        block(end).numSalient(range)  = more.numSalient;
        block(end).numDistract(range) = more.numDistract;
      else
