...
vr.displayMode = eval(vr.exper.variables.displayMode);

vr.currentWorld = eval(vr.exper.variables.startWorld);

[rewardY, rewardFs] = audioread('trill_6k_5k_10k.wav');
vr.rewardPlayer = audioplayer(rewardY, rewardFs);
...
if isReward
  play(vr.rewardPlayer);
  deliverReward(vr, vr.rewardMSec * vr.rewardFactor);
  vr.numRewards = vr.numRewards + 1;
end
...
terminateVRRig(vr);
fprintf('\n\n============>>>>>> %d rewards (%.3g mL) obtained\n\n', vr.numRewards, vr.numRewards*vr.rewardFactor*RigParameters.rewardSize);
