function code = poisson_patches_laser
...
% receive parameters from LaserGalvo PC
      ...
      case BehavioralState.WithinTrial
        ...
        for iHint = 1:numel(vr.choiceHintNames)
          if vr.hintVisibleFrom(iHint) > 2 && vr.stemLength - vr.position(2) <= vr.hintVisibleFrom(iHint) ...
            && ~vr.mazes(vr.mazeID).turnHint_Mem
            triangles         = vr.(vr.choiceHintNames{iHint})(vr.trialType,:);
            vr.worlds{vr.currentWorld}.surface.visible(triangles) = true;
            vr.hintVisibleFrom(iHint)                             = nan;
          end
        end
...
if isempty(vr.laser.currPool); vr.laser.currPool = vr.laser.combinedPool; end

if vr.warmupIndex < 1 && ~vr.easyBlockFlag
  if (numel(vr.laser.epoch) == 1 && (strcmpi(vr.laser.epoch{1},'cueHalf2') || ...
      strcmpi(vr.laser.epoch{1},'cueHalf1')))
    
    deletedStimTrials = 4:4:5000;
    
  elseif numel(vr.laser.epoch) > 1 && ...
      sum(cell2mat(cellfun(@(x)(~isempty(strfind(x,'cueHalf'))),vr.laser.epoch,'uniformOutput',false))) > 0
    deletedStimTrials = [5:5:20000 6:5:20000];
    
  else
    deletedStimTrials = nan;
    
  end
  
  if ismember(vr.trialID,deletedStimTrials)
    vr.laser.lsrON = 0; %  do not turn laser on in ctrl trials where cues have been deleted
  else
    vr.laser.lsrON = binornd(1,vr.laser.P_on,1); % will laser be ON or OFF?
  end
  
else
  vr.laser.lsrON = 0; % laser always off in warmup trials
end
...
if vr.laser.lsrON
  ...
  switch vr.laser.drawMode
    case 'pseudo-random' % draw until all locations have been visited once
      idx                      = randi(size(vr.laser.currPool,2),1);
      vr.laser.pos             = vr.laser.currPool(1,idx);
      vr.laser.currentEpoch    = vr.laser.currPool(2,idx);
...
if ~vr.laser.lsrON
  ...
  switch vr.laser.epoch{vr.laser.currentEpoch}
    case 'whole' % assuming ramp down time doesn' really matter here
    ...
    case 'cue'
      switch vr.laser.rampDownMode
        ...
        case 'predicted' % predicts where the mouse will be in 100 ms based on current velocity
          vr = rampDownPredictor(vr);
          if vr.stateCode == virmenStateCodes.WithinTrial && ...
              vr.iCueEntry > 0                        && ...
              vr.predictedYpos < vr.lCue
            vr.laser.currentState = vr.laser.pos;
          else
            vr.laser.currentState = 0;
          end
      ...
      switch vr.laser.rampDownMode
        ...
        case 'predicted' % predicts where the mouse will be in 100 ms based on current velocity
          ...
          if vr.stateCode == virmenStateCodes.WithinTrial && ...
              vr.iMemEntry     > 0                    && ...
              vr.position(2)   > vr.lCue              && ...
              vr.predictedYpos < vr.stemLength
            vr.laser.currentState = vr.laser.pos;
          else
            vr.laser.currentState = 0;
          end
      ...
      switch vr.laser.rampDownMode
        case 'actual'
          if vr.stateCode == virmenStateCodes.WithinTrial && ...
              vr.iMemEntry     > 0                    && ...
              vr.position(2)   > vr.lCue              && ...
              vr.position(2)   < vr.stemLength - vr.lMemory/2
            vr.laser.currentState = vr.laser.pos;
          else
            vr.laser.currentState = 0;
          end
        case 'predicted' % predicts where the mouse will be in 100 ms based on current velocity
          ...
          if vr.stateCode == virmenStateCodes.WithinTrial && ...
              vr.iMemEntry     > 0                    && ...
              vr.position(2)   > vr.lCue              && ...
              vr.predictedYpos < vr.stemLength - vr.lMemory/2
            vr.laser.currentState = vr.laser.pos;
          else
            vr.laser.currentState = 0;
          end
    ...
    case 'reward'
      if vr.stateCode == virmenStateCodes.DuringReward
        vr.laser.currentState = vr.laser.pos;
      else
        vr.laser.currentState = 0;
      end
    case 'iti'
      if vr.stateCode == virmenStateCodes.InterTrial
        vr.laser.currentState = vr.laser.pos;
      else
        vr.laser.currentState = 0;
      end
...
%%-------------------------------------------------------------------------
% based on curret velocity, predict Y position that is
% vr.laser.rampDownTime seconds away
function vr = rampDownPredictor(vr)

vr.predictedYpos = vr.position(2)+vr.velocity(2)*vr.laser.rampDownDur;
...
vr.laser.nLocations     = numel(vr.laser.locationSet);
vr.laser.nEpochs        = numel(vr.laser.epoch);
...
vr.laser.epochPool      = 1:vr.laser.nEpochs;
epochVec                = arrayfun(@(x)(ones(1,vr.laser.nLocations)*x),vr.laser.epochPool,'UniformOutput',false);
epochVec                = [epochVec{:}];
vr.laser.combinedPool   = [repmat(1:vr.laser.nLocations,[1 vr.laser.nEpochs]) ; epochVec];
vr.laser.currPool       = vr.laser.combinedPool;
