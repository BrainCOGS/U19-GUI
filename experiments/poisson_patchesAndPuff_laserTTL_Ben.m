function code = poisson_patchesAndPuff_laserTTL_Ben
...
function vr = initializationCodeFun(vr)
  ...
  % initialize time of last outcome
  vr.timeLastOutcome = clock;
  % initialize prev trial laser condition
  vr.prevLsrOn =0;
        ...
        if abs(angleMPiPi(vr.position(end))) < pi/2
          ...
          for iSide = 1:numel(ChoiceExperimentStats.CHOICES)
            ...
            if any(isTriggered)
              ...
              % if right side tower, deliver right side puff, 40ms, else left side puff 
              if iSide==2;       
                  nidaqPulse3('ttl',40);               
              else                 
                  nidaqPulse4('ttl',40);               
...
function vr = initializeTrialWorld(vr)
  ...
  vr                        = autoAdjustReward(vr);
      ...
      vr.LaserTrialType = 0; %default

  if vr.warmupIndex < 1 && ~vr.easyBlockFlag && ~vr.prevLsrOn
    
    vr.lsrON = binornd(1,vr.P_on,1); % will laser be ON or OFF?
    
    if vr.lsrON && strcmp(vr.lsrepoch,'Alt_Cue_Outcome') 
      if rand<0.5 %probability half
        vr.LaserTrialType = 1; %laser during cue
      else
        vr.LaserTrialType = 2; %laser during outcome
      end
    end
...
function vr = setupTrials(vr, shaping)
  ...
  cfg.trialData         = { 'trialProb', 'trialType', 'choice', 'trialID'           ...
                          ...
                          , 'iLaserOn','iLaserOff','lsrON','LaserTrialType'};
...
if ~vr.lsrON
    ...
    switch vr.lsrepoch
        ...
        case 'mem'            
            if vr.stateCode == virmenStateCodes.WithinTrial && ...
                    vr.position(2)   > vr.lCue              && ...
                    vr.position(2)   < (vr.lCue+vr.lMemory)
                vr.currentState = 1;
            else
                vr.currentState = 0;
            end
      ...
      case 'Alt_Cue_Outcome'
        if vr.stateCode == virmenStateCodes.WithinTrial && ...
            vr.position(2) > 0                          && ...
            vr.position(2) < vr.lCue                    && ...
            vr.LaserTrialType == 1
          vr.currentState = 1;
        elseif etime(clock,vr.timeLastOutcome)<2 && ...
            vr.LaserTrialType == 2
          vr.currentState = 1;
        else
          vr.currentState = 0;
