...
% To get referencing tool to show up, you need to put the filenames+path
% into the Towers Task GUI, then when you synchronize, the filenames will
% show up in the GUI on the imaging computer. Double clicking on the
% filenames will open up the reference alignment tool.
...
%                             , 'dirName'   , {'DIR...'}            ...
  ...
  methods (Access = protected)
    ...
    function found = findScanImage(obj)
      ...
      set(obj.scanImage.hSICtl.hGUIData.mainControlsV4.fileCounter , 'BackgroundColor', [1 1 1]);
      
    end
    
    %----- Safely set ScanImage file name
    function setScanImageOutput(obj, fileStem)

      existingFiles   = dir(fullfile(obj.scanImage.hSI.hScan2D.logFilePath, [fileStem '_*.tif']));
      existingFiles   = {existingFiles.name};
      acquisitions    = regexp(existingFiles, [fileStem '_([0-9]+)_[0-9]+.tif'], 'tokens', 'once');
      acquisitions    = [acquisitions{:}];
      if isempty(acquisitions)
        acquisitions  = 0;
      else
        acquisitions  = unique(str2double(acquisitions));
      end
      
      obj.scanImage.hSI.hScan2D.logFileStem     = fileStem;
      obj.scanImage.hSI.hScan2D.logFileCounter  = max(acquisitions) + 1;
    ...
    function createGUIFigure(obj, name)
      ...
      obj.figGUI    = makeFigureAt( obj.GUI_POS, obj.GUI_MONITOR, 'OuterPosition'   ...
                                  , 'Name'        , name                            ...
                                  , 'NumberTitle' , 'off'                           ...
                                  , 'Color'       , obj.GUI_COLOR                   ...
                                  , 'Menubar'     , 'none'                          ...
    ...
    function reconnectServer(obj, handle, event)
      ...
      serverIPAddress = get(obj.socket{end}, 'RemoteHost');
      fclose(obj.socket{end});
      delete(obj.socket{end});
      obj.socket{end} = tcpip ( serverIPAddress, obj.HANDSHAKE_PORT             ...
                              , 'NetworkRole'           , 'client'              ...
                              , 'BytesAvailableFcn'     , @obj.fcnNegotiatePort ...
                              , 'BytesAvailableFcnCount', 2                     ...
                              );
    ...
    function fcnClient(obj, message)
      ...
      elseif strcmp(message, obj.BYEBYE)
        ...
        if ~isempty(obj.imgCompare)
          obj.imgCompare.toggleOnline([], [], true);
        end
        obj.disconnectServer();
      ...
      elseif ~isempty(message)
        ...
        if isempty(info)
          ...
          if isempty(dir)
            warning('OutputSynchronizer:safety', 'An absolute output path must be provided, received "%s" instead. Will ignore.', message);
          else
            if ~exist(dir, 'dir')
              mkdir(dir);
            end
          ...
          set(obj.scanImage.hSICtl.hGUIData.mainControlsV4.fileCounter , 'BackgroundColor', obj.COLOR_CONNECT);
        else
          info            = [info; message];
        end
    ...
    function processSyncInfo(obj, handle, event)
      ...
      item        = deblank(strtrim(items{get(handle, 'Value')}));
...
%           item    = [name ext];     % to be copied to clipboard
          ...
          obj.setScanImageOutput([obj.scanImage.basename item]);
        end
%         clipboard('copy', item);
