...
%                       during a run.
%
classdef ExperimentLog_laser < handle
...
%     SENSOR_COORDS   = 1:7;          % Columns of raw sensor readout to store
    ...
    DEFAULT_PREALLOCSIZE_ITER  = 2000   % Default size of arrays to preallocate for variables that grow at every iter, based on max trial duration
  ...
  methods
    ...
    %----- Constructor
    function obj = ExperimentLog_laser(vr, cfg, console, preallocSize)
      ...
      if nargin < 4
        obj.preallocSize      = ExperimentLog_laser.DEFAULT_PREALLOCSIZE;
      else
      ...
      obj.trialInfo.position   = nan(ExperimentLog_laser.DEFAULT_PREALLOCSIZE_ITER, numel(ExperimentLog_laser.SPATIAL_COORDS));
      obj.trialInfo.velocity   = nan(ExperimentLog_laser.DEFAULT_PREALLOCSIZE_ITER, numel(ExperimentLog_laser.SPATIAL_COORDS));
      obj.trialInfo.sensorDots = zeros(ExperimentLog_laser.DEFAULT_PREALLOCSIZE_ITER, numel(ExperimentLog_laser.SENSOR_COORDS), ExperimentLog_laser.SENSOR_DATASIZE);
      obj.trialInfo.collision  = false(0);
      obj.trialInfo.time       = nan(ExperimentLog_laser.DEFAULT_PREALLOCSIZE_ITER,1);
      obj.trialInfo.iterations = uint32(0);
      obj.trialInfo.galvoPosIdx= nan(ExperimentLog_laser.DEFAULT_PREALLOCSIZE_ITER,1);
      obj.trialInfo.lsrTrigIn  = nan(ExperimentLog_laser.DEFAULT_PREALLOCSIZE_ITER,1);
      ...
      for field = ExperimentLog_laser.PREALLOCATED_FIELDS
    ...
    function log = save(obj, compact, timeNow, plots)
      ...
      if obj.writeIndex > 0 && ~isnan(obj.currentTrial.duration)
        ...
        obj.currentTrial.position(obj.currentIt+1:end,:)    = [];
        obj.currentTrial.velocity(obj.currentIt+1:end,:)    = [];
        obj.currentTrial.sensorDots(obj.currentIt+1:end,:)  = [];
        obj.currentTrial.galvoPosIdx(obj.currentIt+1:end,:) = [];
        obj.currentTrial.lsrTrigIn(obj.currentIt+1:end,:)   = [];
      ...
      if nargin > 3
        ...
        if sum(isMissing) > 0
          warning('ExperimentLog_laser:save', '%d figure(s) to be saved are missing, will be skipped.', sum(isMissing));
    ...
    function prevTrialDuration = logStart(obj, vr)
      ...
      if obj.writeIndex > 0
        ...
        obj.currentTrial.time(obj.currentIt+1:end,:)        = [];
        obj.currentTrial.sensorDots(obj.currentIt+1:end,:)  = [];
        obj.trialInfo.galvoPosIdx(obj.currentIt+1:end,:)    = [];
        obj.trialInfo.lsrTrigIn(obj.currentIt+1:end,:)      = [];
    ...
    function logTick(obj, vr, sensorDots)
      ...
      obj.currentTrial.galvoPosIdx(obj.currentIt,1)   = vr.laser.currentState;
      obj.currentTrial.lsrTrigIn(obj.currentIt,:)     = vr.laser.lsrTrigIn;
      
      if nargin > 2 && ~isempty(sensorDots)
        obj.currentTrial.sensorDots(obj.currentIt,:)  = sensorDots(ExperimentLog_laser.SENSOR_COORDS);
      end
      
      if obj.trialEnded <= 1      % Should log the final position at end of trial
        obj.currentTrial.position(obj.currentIt,:)    = vr.position(ExperimentLog_laser.SPATIAL_COORDS);
        obj.currentTrial.velocity(obj.currentIt,:)    = vr.velocity(ExperimentLog_laser.SPATIAL_COORDS);
