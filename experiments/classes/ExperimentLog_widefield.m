...
%                       during a run.
%
classdef ExperimentLog_widefield < handle
...
%     SENSOR_COORDS   = 1:7;          % Columns of raw sensor readout to store
  ...
  methods
    ...
    %----- Constructor
    function obj = ExperimentLog_widefield(vr, cfg, console, preallocSize)
      ...
      if nargin < 4
        obj.preallocSize      = ExperimentLog_widefield.DEFAULT_PREALLOCSIZE;
      else
      ...
      obj.trialInfo.position   = nan(ExperimentLog_widefield.DEFAULT_PREALLOCSIZE_ITER, numel(ExperimentLog_widefield.SPATIAL_COORDS));
      obj.trialInfo.velocity   = nan(ExperimentLog_widefield.DEFAULT_PREALLOCSIZE_ITER, numel(ExperimentLog_widefield.SPATIAL_COORDS));
      obj.trialInfo.sensorDots = zeros(ExperimentLog_widefield.DEFAULT_PREALLOCSIZE_ITER, numel(ExperimentLog_widefield.SENSOR_COORDS), ExperimentLog_widefield.SENSOR_DATASIZE);
      obj.trialInfo.collision  = false(0);
      obj.trialInfo.time       = nan(ExperimentLog_widefield.DEFAULT_PREALLOCSIZE_ITER,1);
      obj.trialInfo.iterations = uint32(0);
      obj.trialInfo.camDataIn  = nan(ExperimentLog_widefield.DEFAULT_PREALLOCSIZE_ITER,1);
      ...
      for field = ExperimentLog_widefield.PREALLOCATED_FIELDS
    ...
    function log = save(obj, compact, timeNow, plots)
      ...
      if obj.writeIndex > 0 && ~isnan(obj.currentTrial.duration)
        ...
        obj.currentTrial.camDataIn(obj.currentIt+1:end,:)   = [];
      ...
      if nargin > 3
        ...
        if sum(isMissing) > 0
          warning('ExperimentLog_widefield:save', '%d figure(s) to be saved are missing, will be skipped.', sum(isMissing));
    ...
    function prevTrialDuration = logStart(obj, vr)
      ...
      if obj.writeIndex > 0
        ...
        obj.currentTrial.time(obj.currentIt+1:end,:)        = [];
        obj.currentTrial.sensorDots(obj.currentIt+1:end,:)  = [];
        obj.trialInfo.camDataIn(obj.currentIt+1:end,:)      = [];
    ...
    function logTick(obj, vr, sensorDots)
      ...
      obj.currentTrial.camDataIn(obj.currentIt,1)     = vr.camDataIn;
      
      if nargin > 2 && ~isempty(sensorDots)
        obj.currentTrial.sensorDots(obj.currentIt,:)  = sensorDots(ExperimentLog_widefield.SENSOR_COORDS);
      end
      
      if obj.trialEnded <= 1      % Should log the final position at end of trial
        obj.currentTrial.position(obj.currentIt,:)    = vr.position(ExperimentLog_widefield.SPATIAL_COORDS);
        obj.currentTrial.velocity(obj.currentIt,:)    = vr.velocity(ExperimentLog_widefield.SPATIAL_COORDS);
