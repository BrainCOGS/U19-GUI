classdef TrainingRegiment < handle
  ...
  properties (Constant)
    ...
    TRIAL_DRAWING       = {'eradeTrial', 'eradeCappedTrial', 'errorWeightedTrial', 'pseudorandomTrial', 'leftOnlyTrial', 'rightOnlyTrial', 'alternateTrial'};
...
%     RGX_NUMBER          = '^\s*([+-]?\s*[0-9]*[.]?[0-9]*)'
    ...
    RGX_DIGIT           = '[0-9]+'
    RGX_ANINAME         = '(^|[/_-.])%s([/_-.])'
    RGX_MATFILE         = '[.]([mM]|[mM][aA][tT])$'
    RGX_STIMBANK        = '(?:^|[/\\])stimulus_trains_([^_]+)[^/\\.]*[.]'
...
%         obj.dataSync.waitForConnections(numDataSync);
      ...
      elseif ~strcmpi(mode, 'overwrite') && exist(filePath, 'file')
        ...
        % HACK : need to load some data from log files
        if ~TrainingRegiment.fastLoad()
          if obj.patchMissingData()
            obj.save();
          end
        end
        return;
      ...
      obj.default.block.easyBlockFlag = false;
      ...
      obj.default.block.protocol      = [];
      obj.default.block.frozenStimuli = [];
      obj.default.block.stimulusBank  = '';
      obj.default.block.stimulusSet   = 1;
      obj.default.block.start         = [];
      ...
      obj.default.animal.stimulusSet              = 1;
      obj.default.animal.refImageFiles            = {};
      obj.default.animal.imagingTags              = {};
      obj.default.animal.mainMazeID               = 1;
      obj.default.animal.autoAdvance              = true;
      obj.default.animal.warmupDrawMethod         = {'TRIAL_DRAWING', 1};
      obj.default.animal.mainDrawMethod           = {'TRIAL_DRAWING', 1};
      ...
      if ~TrainingRegiment.fastLoad()
        obj.valveTimer                = timer ( 'Name'          , 'valve-check'                   ...
                                              , 'StartDelay'    , TrainingRegiment.VALVE_TIMEOUT  ...
      ...
      if RigParameters.hasDAQ && ~TrainingRegiment.fastLoad()
        obj.instruments.motionTimer   = timer ( 'Name'          , 'read-motion'                   ...
                                              , 'BusyMode'      , 'drop'                          ...
    ...
    function delete(obj)
      ...
      if ~isempty(obj.valveTimer) && isvalid(obj.valveTimer)
        stop(obj.valveTimer);
    ...
    function merge(obj, other)
      for exper = fieldnames(other.experiment)'
        if isfield(obj.experiment, exper{:})
          obj.experiment.(exper{:})   ...
                = TrainingRegiment.mergeExperiments ( obj.experiment.(exper{:})       ...
    ...
    function [data, backup] = save(regiment)
      ...
      if ~exist(backupFile, 'file') && exist(dataFile, 'file')
        makepath(backupFile);
        movefile(dataFile, backupFile, 'f');
        backup    = backupFile;
      end
    ...
    function guiSelectAnimal(obj, actionStr, actionFcn, cancelFcn)
      ...
      % Ensure correct code versions on training rigs
      isDirty                 = false;
      if ~isempty(strfind(lower(RigParameters.rig), 'train'))
        isDirty               = TrainingRegiment.ensureCleanRepository();
      end
      ...
      obj.drawGUI();
      if isDirty
        set(obj.figGUI, 'Color', TrainingRegiment.UNCLEAN_COLOR);
      end
    ...
    function success = recordBehavior(obj, animal, log, newBlocks)
      ...
      for iBlock = newBlocks
        ...
        run.block(end).protocol             = newBlock.shapingProtocol;
        run.block(end).frozenStimuli        = newBlock.frozenStimuli;
        run.block(end).stimulusBank         = newBlock.stimulusBank;
        run.block(end).stimulusSet          = newBlock.stimulusSet;
        run.block(end).start                = newBlock.start;
        if isfield(newBlock, 'easyBlockFlag')
          run.block(end).easyBlockFlag      = newBlock.easyBlockFlag;
        end
    ...
    function guessPaths(obj, useAbsolute, verbose)
      ...
      % Omit Matlab installation paths
      matlabPath      = matlabroot;
      searchPath      = searchPath(~strncmp(searchPath, matlabPath, numel(matlabPath)));
...
%                || ~exist(obj.absolutePath(run(iRun).dataFile), 'file')
    ...
    function info = dialogEditAnimal(obj, info, vetoedIDs)
      ...
      hDialog       = figure( 'Name'            , 'Training Schedule'                       ...
                            , 'Units'           , 'pixels'                                  ...
                            , 'Position'        , obj.computeFigurePos([0.8 1])             ...
                            , 'Menubar'         , 'none'                                    ...
                            , 'NumberTitle'     , 'off'                                     ...
                            , 'Resize'          , 'on'                                      ...
                            , 'Color'           , obj.GUI_COLOR                             ...
...
%                             , 'WindowStyle'     , 'modal'                                   ...
    ...
    function imports = dialogImportData(obj)
      ...
      for iAni = 1:numel(srcAnimal)
        ...
        if isempty(iTarget)
          ...
          rewardSize(end+1) = source.rewardSize(iAni);
    ...
    function fcnSubmitAction(obj, handle, event)
      ...
      % Disconnect communications for performance reasons
      if ~isempty(obj.dataSync)
        obj.dataSync.disconnect();
      end
    ...
    function fcnCheckRepository(obj, handle, event, launchDir)
      ...
      prevDir         = TrainingRegiment.gotoGitLocation();
    ...
    function fcnSelectAnimal(obj, handle, event, iAni, iSession, hFigure, altCallback)
      ...
      if ~isempty(obj.dataSync)
        ...
        for iTag = 1:numel(obj.animal(iAni).imagingTags)
          obj.dataSync.sync(true, obj.animal(iAni).imagingTags{iTag});
        end
    ...
    function data = fixPaths(obj, data, name, searchPath, useAbsolute, verbose)
      ...
      elseif    ischar(data)                                                  ...
        ...
        elseif ~isempty(regexp(candidate, TrainingRegiment.RGX_MATFILE, 'once'))
    ...
    %----- HACK: to patch summary info using log file contents
    function numPatched = patchMissingData(obj)
      ...
      for iAni = 1:numel(obj.animal)
        for iDay = 1:numel(obj.animal(iAni).data)
          run               = obj.animal(iAni).data(iDay).run;
          for iRun = 1:numel(run)
            logFile         = obj.absolutePath(run(iRun).dataFile);
            needProtocol    = any(cellfun(@isempty, {run(iRun).block.protocol})) || any(cellfun(@isempty, {run(iRun).block.stimulusBank}));
            needClockTime   = any(cellfun(@isempty, {run(iRun).block.start}));
            if (needProtocol || needClockTime) && exist(logFile, 'file')
              load(logFile);

              numPatched    = numPatched + 1;
              
              if needProtocol
                hasStimBank = isfield(log.block, 'stimulusBank');
                hasStimSet  = isfield(log.block, 'stimulusSet');
                for iBlock = 1:numel(run.block)
                  run(iRun).block(iBlock).protocol        = obj.animal(iAni).protocol;
                  if hasStimBank && ~isempty(log.block(iBlock).stimulusBank)
                    run(iRun).block(iBlock).stimulusBank  = log.block(iBlock).stimulusBank;
                  else
                    run(iRun).block(iBlock).stimulusBank  = ['stimulus_trains_' func2str(run(iRun).block(iBlock).protocol) '.mat'];
                  end
                  if hasStimSet && isempty(log.block(iBlock).stimulusSet)
                    run(iRun).block(iBlock).stimulusSet   = log.block(iBlock).stimulusSet;
                  else
                    run(iRun).block(iBlock).stimulusSet   = 1;
                  end
                end
              end
              ...
              if needClockTime
                for iBlock = 1:numel(run.block)
                  run(iRun).block(iBlock).start     = log.block(iBlock).start;
                end
              end
            end
            
            for iBlock = 1:numel(run.block)
              blockProtocol = func2str(run(iRun).block(iBlock).protocol);
              stimProtocol  = regexp(run(iRun).block(iBlock).stimulusBank, TrainingRegiment.RGX_STIMBANK, 'tokens', 'once');
              if ~strcmp(stimProtocol, blockProtocol)
                run(iRun).block(iBlock).protocol        = str2func(stimProtocol{:});
                numPatched  = numPatched + 1;
              end
              if isempty(run(iRun).block(iBlock).frozenStimuli)
                defaultExt  = [blockProtocol '.mat'];
                run(iRun).block(iBlock).frozenStimuli       ...
                            = ~isempty(run(iRun).block(iBlock).stimulusBank)                                            ...
                           && ( numel(run(iRun).block(iBlock).stimulusBank) < numel(defaultExt)                         ...
                             || ~strcmp(run(iRun).block(iBlock).stimulusBank(end-numel(defaultExt)+1:end), defaultExt)  ...
                              );
                numPatched  = numPatched + 1;
              end
            end
          end
          obj.animal(iAni).data(iDay).run           = run;
        end
      end
  ...
  methods (Static)
    ...
    %----- HACK : skip application of default values, for faster loading times. Use at your own risk!!
    function yes = fastLoad(yes)
      persistent  doFastLoad;
      if isempty(doFastLoad)
        doFastLoad  = false;
      end
      if nargin > 0
        doFastLoad  = yes;
    ...
    function obj = loadobj(frozen)
      ...
      if isfield(frozen.animal, 'imagingDepth')
        for iAni = 1:numel(frozen.animal)
          frozen.animal(iAni).imagingTags = {sprintf('%.3gum', frozen.animal(iAni).imagingDepth)};
        end
        frozen.animal   = rmfield(frozen.animal, 'imagingDepth');
      end
      if isfield(frozen.animal, 'trialDrawMethod')
        for iAni = 1:numel(frozen.animal)
          frozen.animal(iAni).warmupDrawMethod  = frozen.animal(iAni).trialDrawMethod;
          frozen.animal(iAni).mainDrawMethod    = frozen.animal(iAni).trialDrawMethod;
        end
        frozen.animal   = rmfield(frozen.animal, 'trialDrawMethod');
      end
      ...
      if isfield(frozen.animal, 'virmenFrictionCoeff')
        frozen.animal   = rmfield(frozen.animal, 'virmenFrictionCoeff');
      end
      ...
      if ~TrainingRegiment.fastLoad()
        for name = fieldnames(frozen.experiment)'
          exper           = frozen.experiment.(name{:});
          [~,~,iVersion]  = unique([exper.version]);
          experiment      = [];
          for iVer = 1:max(iVersion)
            source        = exper(iVersion == iVer);
            target        = struct();
            for field = fieldnames(source)'
              data        = {source.(field{:})};
              data(isempty(data))     = [];
              if any(cellfun(@isstruct, data))
                data      = [data{:}];
                if isempty(data)
                  target.(field{:})   = data;
                else
                  [~,iData] = unique(cat(1, data.date), 'rows');
                  target.(field{:})   = data(iData);
                end
              else
                target.(field{:})     = data{1};
              end
            ...
            if isempty(experiment)
              ...
              experiment(end+1)       = target;
            end
          end
          frozen.experiment.(name{:}) = experiment;
        end
      end
    ...
    %----- Call this before issuing git commands, so as to refer to the repository for this code
    function prevDir = gotoGitLocation()
      location    = parsePath(which('TrainingRegiment'));
      location    = parsePath(parsePath(location));
      prevDir     = cd(location);
    end
    ...
    %----- Reset git repository to the last commit in the current branch if neccessary
    function isDirty = ensureCleanRepository()
      ...
      if status ~= 0 || ~ isempty(out)
        %% Confirm with user that the repository should be reset
        discard           = questdlg('WARNING:  git repository has uncommitted changes. Discard?', 'Reset git repository', 'Yes', 'No', 'Yes');
        if strcmp(discard, 'Yes')
          ...
          [status,out]    = system('git status --porcelain');
          if status ~= 0 || ~isempty(out)
            error('TrainingRegiment:ensureCleanRepository', 'Failed to reset git repository: %s', out);
          end
        else
          isDirty         = true;
        end
    ...
    function fcn = deduceValidator(name)
      ...
      elseif ~isempty(regexp(name, '(Files|Tags)$', 'once'))
    ...
    function data = deduceData(name, handle)
      ...
      elseif ~isempty(regexp(name, TrainingRegiment.RGX_HASNAME, 'once'))
        data    = value;
      elseif ~isempty(regexp(name, 'Tags$|Files$', 'once'))
    ...
    %----- Merge experiment histories
    function target = mergeExperiments(target, source)
      
      for iSrc = 1:numel(source)
        iTarget           = find([target.version] == source(iSrc).version, 1, 'first');
        if isempty(iTarget)
          iTarget         = numel(target) + 1;
          for name = fieldnames(source(iSrc))'
            target(iTarget).(name{:})   = source(iSrc).(name{:});
          end
        else
          for name = fieldnames(source(iSrc))'
            if ~isstruct(source(iSrc).(name{:}))
              continue;
            end
            if isfield(target(iTarget), name{:})
              data        = [target(iTarget).(name{:}), source(iSrc).(name{:})];
              if isempty(data)
                target(iTarget).(name{:}) = data;
              else
                [~,iData] = unique(cat(1, data.date), 'rows');
                target(iTarget).(name{:}) = data(iData);
              end
            else
              target(iTarget).(name{:})   = source(iSrc).(name{:});
            end
