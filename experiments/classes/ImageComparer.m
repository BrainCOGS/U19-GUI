...
%     GUI_POS         = [-400 44 400 -50]
  ...
  methods
    ...
    function setRefImage(obj, refFile, maxFrames)
      ...
      numFrames         = min(maxFrames, numel(refInfo));
      refStack          = zeros(refInfo(1).Height, refInfo(1).Width, numFrames);
      for iFrame = 1:numFrames
        refStack(:,:,iFrame)  = imread(refFile, iFrame);
      end
      obj.refImage      = mean(refStack, 3);
      clear('refStack');
...
%       profile viewer;
    ...
    function toggleOnline(obj, handle, event, status)
