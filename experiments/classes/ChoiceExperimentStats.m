classdef ChoiceExperimentStats < handle
  ...
  properties (Constant)
    ...
    NTRIALS_DEBIAS    = 10                  % Minimum number of trials per choice before de-biasing is applied
...
%       probability   = max(min( sqrt(1 - correctFrac) , 1-obj.MIN_PROBABILITY), obj.MIN_PROBABILITY);
      probability   = max(min( 1 - correctFrac , 1-obj.MIN_PROBABILITY), obj.MIN_PROBABILITY);
    ...
    % Rosenberger, W. F., Stallard, N., Ivanova, A., Harper, C. N. & Ricks, M. L. Optimal Adaptive
    % Designs for Binary Response Trials. Biometrics 57, 909–913 (2001).  
    %
    % Hu, F., Zhang, L.-X. & He, X. Efficient randomized-adaptive designs. Ann. Statist. 37,
    % 2543–2560 (2009). 
    %
...
%       assignFrac    = numel(obj.typeIndex{1}) / sum(cellfun(@numel,obj.typeIndex));
    ...
    %----- Same as eradeTrial() except with a bug fix so that the right/left drawing probabilities are correctly capped 
    function [success, probability] = eradeCappedTrial(obj, maze, alpha, varargin)
...
%       assignFrac    = numel(obj.typeIndex{1}) / sum(cellfun(@numel,obj.typeIndex));
      ...
      prob1         = max(min( prob1, 1-obj.MIN_PROBABILITY), obj.MIN_PROBABILITY);
    ...
    %----- Alternate between left and right trials
    function [success, probability] = alternateTrial(obj, maze, varargin)
      if obj.currentTrial==0
        [success, probability] = obj.newTrial(maze, Choice.L);
      elseif obj.trialChoice(obj.currentTrial)==1
        [success, probability] = obj.newTrial(maze, Choice.R);
      else
        [success, probability] = obj.newTrial(maze, Choice.L);
      end  
  ...
  methods (Access = protected)
    ...
    function correctFrac = weightedCorrectness(obj, maze)
      ...
      if    obj.currentTrial < 1                  ...
        ||  obj.mazeID(obj.currentTrial) ~= maze  ...
        ||  any(cellfun(@numel, obj.typeIndex) < ChoiceExperimentStats.NTRIALS_DEBIAS)
        correctFrac           = [];
        return;
      end
