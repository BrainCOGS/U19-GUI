...
%% delete 1st half of cues every md stimulus for matched control to laser trials
md = 5;
...
for jj = 1:size(poissonStimuliTemp.panSession,1) % maze
    for ii = md:md:size(poissonStimuliTemp.panSession,2) % stim ID
        ...
        idxL = find(poissonStimuliTemp.panSession(jj,ii).cuePos{1} < poissonStimuliTemp.config(jj).lCue/2);
        idxR = find(poissonStimuliTemp.panSession(jj,ii).cuePos{2} < poissonStimuliTemp.config(jj).lCue/2);
        
        poissonStimuliTemp.panSession(jj,ii).cuePos{2}(idxR) = [];
        poissonStimuliTemp.panSession(jj,ii).cuePos{1}(idxL) = [];
        ...
        idxR2 = find(poissonStimuliTemp.panSession(jj,ii).cueCombo(2,:)==1);
        idxL2 = find(poissonStimuliTemp.panSession(jj,ii).cueCombo(1,:)==1);
        poissonStimuliTemp.panSession(jj,ii).cueCombo(2,idxR2(idxR)) = 0;
        poissonStimuliTemp.panSession(jj,ii).cueCombo(1,idxL2(idxL)) = 0;
    ...
    for ii = md:md:size(poissonStimuliTemp.perSession,2) % stim ID
        for kk = 1:size(poissonStimuliTemp.perSession,3)
            ...
            idxR = find(poissonStimuliTemp.perSession(jj,ii,kk).cuePos{2} > poissonStimuliTemp.config(jj).lCue/2);
            idxL = find(poissonStimuliTemp.perSession(jj,ii,kk).cuePos{1} < poissonStimuliTemp.config(jj).lCue/2);
            
            poissonStimuliTemp.perSession(jj,ii,kk).cuePos{2}(idxR) = [];
            poissonStimuliTemp.perSession(jj,ii,kk).cuePos{1}(idxL) = [];
            ...
            idxR2 = find(poissonStimuliTemp.perSession(jj,ii,kk).cueCombo(2,:)==1);
            idxL2 = find(poissonStimuliTemp.perSession(jj,ii,kk).cueCombo(1,:)==1);
            poissonStimuliTemp.perSession(jj,ii,kk).cueCombo(2,idxR2(idxR)) = 0;
            poissonStimuliTemp.perSession(jj,ii,kk).cueCombo(1,idxL2(idxL)) = 0;
...
%% delete 2nd half of cues every md stimulus for matched control to laser trials
badidx_pan = []; badidx_per = [];
for jj = 1:size(poissonStimuliTemp.panSession,1) % maze
    for ii = md+1:md:size(poissonStimuliTemp.panSession,2) % stim ID
        
        % copy previous stim
        poissonStimuliTemp.panSession(jj,ii) = poissonStimuliTemp.panSession(jj,ii-2);
    ...
    for ii = md+1:md:size(poissonStimuliTemp.perSession,2) % stim ID
        for kk = 1:size(poissonStimuliTemp.perSession,3)
            % copy previous stim
            poissonStimuliTemp.perSession(jj,ii,kk) = poissonStimuliTemp.perSession(jj,ii-2,kk);
...
for ii = 1:size(badidx_pan,1)
    if badidx_pan(ii,2) >=9
        if badidx_pan(ii,1)+md*2 <= size(poissonStimuliTemp.panSession,2)
            poissonStimuliTemp.panSession(badidx_pan(ii,2),badidx_pan(ii,1)) = poissonStimuliTemp.panSession(badidx_pan(ii,2),badidx_pan(ii,1)+md*2);
        else
            poissonStimuliTemp.panSession(badidx_pan(ii,2),badidx_pan(ii,1)) = poissonStimuliTemp.panSession(badidx_pan(ii,2),badidx_pan(ii,1)-md*2);
        end
...
save stimulus_trains_PoissonBlocksReboot3m_laser_deleteHalf1or2_20sets_dup0p5_pan100_n400_x4.mat poissonStimuli