function code = poisson_patches_widefield
...
function vr = initializationCodeFun(vr)
  ...
  vr    = initializeVRRig_widefield(vr, vr.exper.userdata.trainee);
  %nidaqPulse2('ttl',200); % start image acquisition (send start trigger)
...
try
  ...
  switch vr.state           % ... take action depending on the simulation state
    ...
    case BehavioralState.InitializeTrial
      ...
      if ~vr.shutterIsOpen
          vr.shutterIsOpen = 1;
          nidaqPulse2('ttl',200); % turn cam on
          vr.iCamOn = vr.iterFcn(vr.logger.iterationStamp(vr));
          nidaqPulse3('on'); % open shutter
      end
  ...
  % read sync signal from camera
  temp         = nidaqDIread('readDI');
  vr.camDataIn = temp(1);
...
function vr = setupTrials(vr, shaping)
  ...
  cfg.blockData         = { 'mazeID', 'mainMazeID', 'motionBlurRange', 'iterStr'    ...
                          ...
                          , 'stimulusConfig', 'stimulusSet'  , 'frozenStimuli'};
  ...
  if ~vr.experimentEnded
    vr.logger           = ExperimentLog_widefield(vr, cfg, vr.protocol, vr.iterFcn(inf));
  end
