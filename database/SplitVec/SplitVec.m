...
%SAK groupfun = @(v) any(diff(v,1,1)~=0,2);
...
function result = groupfun(v)
  result = any(diff(v,1,1)~=0,2);
end

function results = consecfun(v)
  results = diff(v(:,1)-(1:size(v,1))')~=0;
end

function results = reversefun(v)
  results = diff(v(:,1)-(size(v,1):-1:1)')~=0;
end
