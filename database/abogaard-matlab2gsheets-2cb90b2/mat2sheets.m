function [status, sheetID, sheetData] = mat2sheets(spreadsheetID, sheetID, sheetpos, d, newTitle)
...
%       newTitle:       optional new title for the sheet
%
% Alternative syntaxes (hacked by SAK):
%   sheetData = mat2sheets(spreadsheetID);                    % get info about the spreadsheet only
%   status = mat2sheets(spreadsheetID, 'some title');         % create a new sheet with this title
%   status = mat2sheets(spreadsheetID, ['!' sheetID]);        % delete this sheet
%   status = mat2sheets(spreadsheetID, sheetID, 'new title'); % change title of sheet only
%   status = mat2sheets(spreadsheetID, sheetID, [0 -3]);      % delete column 3
%   status = mat2sheets(spreadsheetID, sheetID, [-2 0]);      % delete row 2
%
%
...
%
% SAK : This is the most useful reference for how to manipulate Google spreadsheets remotely:
...
if isempty(aSheets)
  error('mat2sheets:refreshAccessToken', 'Cannot do anything without Google API access. See readme'); 
  return; 
end

% SAK: I hate this code, it's impossible to modify other than via hacking
if nargin < 2
  [~,~,~,~,status,aSheets] = getSheetMetaData(spreadsheetID, '0', aSheets);
  return;
end
if nargin < 3
  if sheetID(1) == '!'
    [status, aSheets] = deleteSheet(spreadsheetID, sheetID(2:end), aSheets);
  else
    [status, aSheets] = addNewSheet(spreadsheetID, sheetID, aSheets);
  end
  return;
end
if nargin < 4       % change title or delete rows/columns only
  if ischar(sheetpos)
    [status, aSheets] = appendRowsColumns(spreadsheetID, sheetID, 0, 0, aSheets, sheetpos);
  else
    [status, aSheets] = deleteRowsColumns(spreadsheetID, sheetID, -sheetpos(1), -sheetpos(2), aSheets);
  end
  return;
end
if nargin < 5
...
[nR, nC, sheetName, sheetID, sheetData, aSheets] = getSheetMetaData(spreadsheetID, sheetID, aSheets); % to ensure we have enough cells
maxR = sheetpos(1)+size(d,1)-1; % total theoretical rows needed
maxC = sheetpos(2)+size(d,2)-1; % '' cols needed

if ~isempty(nR) || ~isempty(newTitle) % is possible, check that the sheet is large enough
    [status, aSheets] = appendRowsColumns(spreadsheetID, sheetID, maxR-nR, maxC-nC, aSheets, newTitle);
end

%update sheet
[status, aSheets]  = pasteSheetData(spreadsheetID, sheetName, sheetpos, d, aSheets);
if isnumeric(sheetID)
  sheetID = int2str(sheetID);
end

%--------------------------------------------------------------------------
function [status, aSheets] = addNewSheet(spreadsheetID, title, aSheets)
...
while ~success && iter<maxiter    
    ...
    if (con.getResponseCode()~=200)
        ...
        if con.getResponseCode()==401
          aSheets = refreshAccessToken(true);
        end
        continue;
    end
    success=true;
...
if ~success
    status = 0;
    error('mat2sheets:addNewSheet', ['Failed trying to add a new sheet. Last response was: ' num2str(con.getResponseCode) '/' con.getResponseMessage().toCharArray()']);
end


%--------------------------------------------------------------------------
function [status, aSheets] = deleteSheet(spreadsheetID, sheetID, aSheets)
...
if ~success
    status = 0;
    error('mat2sheets:deleteSheet', ['Failed trying to delete sheet ' sheetID '. Last response was: ' num2str(con.getResponseCode) '/' con.getResponseMessage().toCharArray()']);
end


%--------------------------------------------------------------------------
function [status, aSheets] = appendRowsColumns(spreadsheetID, sheetID, rowsadd, colsadd, aSheets, newTitle)
...
if ~success
    status = 0;
    error('mat2sheets:appendRowsColumns', ['Failed trying to add rows/columns. Last response was: ' num2str(con.getResponseCode) '/' con.getResponseMessage().toCharArray()']);
end

%--------------------------------------------------------------------------
function [status, aSheets] = deleteRowsColumns(spreadsheetID, sheetID, rowdel, coldel, aSheets)
    ...
    request{end+1} = ['''requests'': [{',...
                          '''deleteDimension'': {',...
                            '''range'': {',...
                                '''sheetId'': ' sheetID ',',...
                                '''dimension'': ''ROWS'',',...
                                '''startIndex'': ' num2str(rowdel-1) ',',...
                                '''endIndex'': ' num2str(rowdel),...
                            '}',...
                          '}',...
                      '}]'];
end
    ...
    request{end+1} = ['''requests'': [{',...
                          '''deleteDimension'': {',...
                            '''range'': {',...
                                '''sheetId'': ' sheetID ',',...
                                '''dimension'': ''COLUMNS'',',...
                                '''startIndex'': ' num2str(coldel-1) ',',...
                                '''endIndex'': ' num2str(coldel),...
                            '}',...
                          '}',...
                      '}]'];
end
...
if ~success
    status = 0;
    error('mat2sheets:deleteRowsColumns', ['Failed trying to delete rows/columns. Last response was: ' num2str(con.getResponseCode) '/' con.getResponseMessage().toCharArray()']);
end

%--------------------------------------------------------------------------
function [status, aSheets] = pasteSheetData(spreadsheetID, sheetName, sheetpos, d, aSheets)
...
while ~success && iter<maxiter    
    ...
    con.setRequestProperty('X-Upload-Content-Length', '0');
    event = ['{',...
                '''valueInputOption'': ''USER_ENTERED'',',...     % SAK : want interpretation!
                '''data'': [{',...
...
if ~success
    status = 0;
    error('mat2sheets:pasteSheetData', ['Failed pasting data. Last response was: ' num2str(con.getResponseCode) '/' con.getResponseMessage().toCharArray()']);
end
...
%--------------------------------------------------------------------------
function [nRows, nCols, sheetName, sheetID, jdat, aSheets] = getSheetMetaData(spreadsheetID, sheetID, aSheets)
...
if success
    ...
    %% SAK : consolidate sheets structures
    if iscell(jdat.sheets)
      sheets = struct([]);
      for iSheet = 1:numel(jdat.sheets)
        for field = fieldnames(jdat.sheets{iSheet})'
          sheets(iSheet).(field{:}) = jdat.sheets{iSheet}.(field{:});
        end
      end 
      jdat.sheets = sheets;
    end
    ...
    %%
    if sheetID(1) == '!'
      %% SAK : Look up sheet by title
      sheetProps = [jdat.sheets.properties];
      wheresheet = find(strcmp({sheetProps.title}, sheetID(2:end)));
      if isempty(wheresheet)
        error('mat2sheets:title', 'No sheet with title "%s" found for spreadsheet %s.', sheetID, spreadsheetID);
      elseif numel(wheresheet) ~= 1
        error('mat2sheets:title', 'Multiple sheets with title "%s" found for spreadsheet %s.', sheetID, spreadsheetID);
      else
        sheetID = int2str(sheetProps(wheresheet).sheetId);
      end
    ...
    if ~isempty(wheresheet) && length(wheresheet)==1
        ...
        error('mat2sheets:getSheetMetaData', 'SheetID doesnt match sheets');
    end
    
    con.disconnect();
    
else
    error('mat2sheets:getSheetMetaData', ['Failed gathering metadata. Last response was: ' num2str(con.getResponseCode) '/' con.getResponseMessage().toCharArray()']);
end

%--------------------------------------------------------------------------
function aSheets = refreshAccessToken(force)
...
if ~exist('google_tokens.mat', 'file')
    error('mat2sheets:refreshAccessToken', 'Run RunOnce() first and ensure that google_tokens.mat is in MATLAB''s path'); 
end

load google_tokens.mat
if ~force && toc(tAccess) < 0.9*expires_in    % SAK : don't keep on pinging when we have a perfectly valid token
  return;
end
...
for i=1:length(reply_commas)-1
    ...
    elseif ~isempty(strfind(newAccessTokenString(reply_commas(i):reply_commas(i+1)),'expires_in'))
        tmp=newAccessTokenString(reply_commas(i):reply_commas(i+1));
        index_tmp_colon=strfind(tmp,':');
        tmp=tmp(index_tmp_colon+1:end); clear index_tmp_colon;
        expires_in=str2double(tmp); clear tmp;
    end
end

save('google_tokens.mat', 'aSheets', 'tAccess', 'expires_in', '-append');
