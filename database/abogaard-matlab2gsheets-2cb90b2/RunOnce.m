...
[aSheets,rSheets,tSheets,tAccess,expires_in] = getAccessToken(client_id, client_secret, scope_sheets);

% SAK added specific location for tokens file
tokensFile = fullfile( fileparts(mfilename('fullpath')), 'google_tokens.mat' );
save(tokensFile, 'client_id', 'client_secret', 'aSheets', 'rSheets', 'tSheets', 'tAccess', 'expires_in');


function [access_token,refresh_token,token_type,tAccess,expires_in]=getAccessToken(client_id, client_secret, scope)
% function by Claudiu
...
input('Press enter when the above steps were successfully completed to continue ');
...
for i=1:length(reply_commas)-1
    ...
    if ~isempty(strfind(accessTokenString(reply_commas(i):reply_commas(i+1)),'expires_in'))
        tmp=accessTokenString(reply_commas(i):reply_commas(i+1));
        index_tmp_colon=strfind(tmp,':');
        tmp=tmp(index_tmp_colon+1:end); clear index_tmp_colon;
